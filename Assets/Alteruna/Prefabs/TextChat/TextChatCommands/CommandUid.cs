...
namespace Alteruna.TextChatCommands
{
	public class CommandUid : ITextChatCommand
	{
		...
		public static CommunicationBridgeUID LastTarget = null;
		...
		public string Usage { get; } = "/uid target(optional) <search>(optional)";
		...
		public string Execute(TextChatSynchronizable textChat, string[] args)
		{
			...
			if (target)
			{
				if (useSearch)
				{
					var components = CommandTarget.LastTarget.gameObject.GetComponents<CommunicationBridgeUID>();
				...
				{
					if (CommandTarget.LastTarget.gameObject.TryGetComponent(out CommunicationBridgeUID u))
					{
						LastTarget = u;
